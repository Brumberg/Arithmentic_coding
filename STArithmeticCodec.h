#ifndef _ST_ARITHMETIC_CODEC_
#define _ST_ARITHMETIC_CODEC_
namespace STArithmeticCodec {
		...
		eErrorCode Compress(size_t len, TDataType *buffer, size_t maxoutlen, T *result, size_t *pOutplen, size_t *pMask)
		{
			...
			if (len&&maxoutlen)
			{
				...
				if (outp<maxoutlen)
				{
					...
					if (longrange==0)
					{
						range = m_Tracker.GetSymbolRange(CEntropyTracker<T,TDataType,dictsize>::EOFIndicator);
						lower = m_Tracker.GetSymbolStatistics(CEntropyTracker<T,TDataType,dictsize>::EOFIndicator);
						upper = m_Tracker.GetSymbolStatistics(CEntropyTracker<T,TDataType,dictsize>::EOFIndicator)+range;
